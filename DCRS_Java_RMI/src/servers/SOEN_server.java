...
public class SOEN_server{
	...
	static HashMap<String, HashMap<String , Integer>> soen_sem_details= new HashMap<String, HashMap<String , Integer>>();
	...
	public static void main(String[] args) throws Exception {
		...
		registry.bind("SOEN_interface", stub);
		System.out.println("SOEN Server is started");
		DatagramSocket aSocket = null;
		
		String course_name = "SOEN";
		...
		for(int i =1 ;i<=3;i++){
			for(int j =1;j<=2;j++){
				if(i==1){
					...
					soen_sem_details.put(semester_name, fall_course_details);
				}
				else if(i==2){
					semester_name = "WINTER";
					winter_course_details.put(course_name+j, i+j);
					soen_sem_details.put(semester_name, winter_course_details);
				}
				else{
					semester_name = "SUMMER";
					summer_course_details.put(course_name+j, i+j);
					soen_sem_details.put(semester_name, summer_course_details);
				}	
			}
		}
		System.out.println(soen_sem_details);
		try {
			aSocket = new DatagramSocket(2964);
			...
			while (true) {
				...
				try{
					if(enroll_request.length==1){
						HashMap<String, Integer> data = soen_sem_details.get(req);
						data1=data.toString().getBytes();
					}
					else{
						System.out.println("BEFORE : "+soen_sem_details);
						if(soen_sem_details.get(enroll_request[0]).containsKey(enroll_request[1])==false){
							System.out.println("ERROR : no course available for "+enroll_request[0]+" term");
							data1 = "Failure : No course".getBytes();
						}
						else{
							int seat = soen_sem_details.get(enroll_request[0]).get(enroll_request[1]);
							if(seat==0){
								System.out.println("ERROR : no seats available");
								data1 = "Failure : No seat".getBytes();
							}
							else{
								soen_sem_details.get(enroll_request[0]).put(enroll_request[1],seat-1);
								System.out.println("Success : Course size reduced by 1");
								System.out.println("AFTER : "+soen_sem_details);
	...
	public void delSemDetails(String course_name, String sem){
		if(soen_sem_details.get(sem).get(course_name)!=null)
			soen_sem_details.get(sem).remove(course_name);
		else
